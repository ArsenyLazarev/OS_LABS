...
void PrintColored(const char* text, int color = 7) {
    setlocale(LC_ALL, "ru");
    static bool useWinApi = true; 
    static bool initialized = false;

    if (!initialized) {
        char choice;
        cout << "Выберите способ вывода (1 - WinAPI цветной, 2 - стандартный): ";
        cin >> choice;
        useWinApi = (choice == '1');
        initialized = true;
    }
    ...
    if (useWinApi) {
        ...
        DWORD charsWritten;
        WriteConsoleA(hConsole, text, strlen(text), &charsWritten, NULL);
...
DWORD WINAPI marker(LPVOID nu) {
    ...
    while (true) {
        ...
        if (arr[a] == 0) {
            ...
            char buffer[100];
            sprintf_s(buffer, "%d %d %d", num, k, a);
            PrintColored(buffer, num + 9); 
...
int main()
{
    ...
    while (number > 0) {
        ...
        {
            PrintColored("Wait for multiple objects failed.", 12); 
            PrintColored("Press any key to exit.", 12);
        }
        EnterCriticalSection(&cs);
        for (int i = 0; i < n; i++) {
            cout << arr[i] << " ";
        ...
        LeaveCriticalSection(&cs);
        cout << "\nType a number of marker that you wanna delete: ";
        cin >> end;
        rem = end;
